  ...
  search: function(cartaClicada) {
    ...
    if(cartaProcurada > cartaDoNaipe) {
        ...
        for(var i = meio; i > 0; i--, j--) {
          ...
          cartaVirada.setAttribute('src', ANIMATION.naipe[j-1][0]);
        ...
        if(primeiraVez) {
          carta = ANIMATION.naipe[Math.floor(meio + ANIMATION.indiceBinaria/2)][1];
          src = ANIMATION.naipe[Math.floor(meio + ANIMATION.indiceBinaria/2)][0];
        } else {
      ...
      ANIMATION.cartas.setAttribute('data-card', carta);
      var src = ANIMATION.naipe[Math.floor(meio/2)][0];
